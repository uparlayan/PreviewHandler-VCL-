...
///  procedure Goster;
///  begin
///    aPreviewer := TPreviewHandler.Create(Form1, aPanel);
///    aPreviewer.FileName := 'C:\aFile.pdf';
///    ShowMessage('Dosya gösterildi');
///    aPreviewer.Destroy;
///  end;
///  </code>
///  </example>
///	<remarks>
///	  Copyright (C) Uğur PARLAYAN (01.05.2013) Lütfen kaynak belirterek kullanınız.
///	</remarks>
...
begin
  ...
  try
    ...
    if (aRegistry.KeyExists(aRegPath) = TRUE) then begin
        aRegistry.Access := KEY_READ or KEY_WOW64_64KEY; { 32 ve 64 bit windows işletim sistemlerinde registry'e erişim farklı kaynaklardan gerçekleşiyor... }
...
begin
  ...
  try
    ...
    if (Mesaj <> '') then begin
        ShowMessage('DoPreview ' + Mesaj); { Bu noktada ShowMessage kullanabilirsiniz, özel kütüphanem nedeniyle ben böyle kullandım. }
    end;
      ...
      if (FControl.Visible = TRUE) then begin
          if (Assigned(FHandler) = True) then begin
              try
                aRect := Rect( 0, 0, FControl.Width, FControl.Height);
                FHandler.SetWindow(FControl.Handle, aRect);
                // FHandler.SetRect(aRect); // gereksiz...
                ...
                on E: Exception do ShowMessage('Hata: ' + E.Message); { Bu noktada ShowMessage kullanabilirsiniz, özel kütüphanem nedeniyle ben böyle kullandım. }
              end;
          end;
      end;
  end;
